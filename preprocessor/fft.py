...
import shutil
import numpy as np
from preprocessor.filtering import run_filter_preprocessor
...
def group_into_bands(fft, fft_freq, nfreq_bands):
    ...
    return df.fft[1:-1] # exclude the band outside frequency band
    # return df.fft
...
def compute_fft(x, data_length_sec, sampling_frequency, nfreq_bands, win_length_sec, stride_sec, features):
    ...
    x2 = np.zeros((n_channels, n_fbins, int(n_timesteps))) #(16, 9, 20)
